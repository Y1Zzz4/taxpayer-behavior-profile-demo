(() => {
  ...
  function renderGraph(data) {
    ...
    function project(node) { let x = node.x, y = node.y, z = node.z; const cy = Math.cos(rotY), sy = Math.sin(rotY), cx = Math.cos(rotX), sx = Math.sin(rotX); const x1 = x * cy - z * sy, z1 = x * sy + z * cy; const y1 = y * cx - z1 * sx, z2 = y * sx + z1 * cx; const perspective = 760 / (900 + z2); return {x: canvas.clientWidth / 2 + x1 * perspective * zoom, y: canvas.clientHeight / 2 + y1 * perspective * zoom, scale: perspective * zoom, depth: z2}; }
