(() => {
  ...
  function renderGraph(data) {
    ...
    function resetStepDemo() { demoActive = false; demoStep = 0; demoPhase = -1; replayButton?.classList.remove('active'); if (replayButton) replayButton.textContent = '开始分步推导 →'; updateDerivationStatus(0); }
    canvas.addEventListener('pointerdown', event => { drag = true; dragDistance = 0; px = event.clientX; py = event.clientY; canvas.setPointerCapture(event.pointerId); }); canvas.addEventListener('pointermove', event => { if (!drag) return; const dx = event.clientX - px, dy = event.clientY - py; dragDistance += Math.hypot(dx, dy); rotY += dx * .006; rotX = Math.max(-1, Math.min(1, rotX + dy * .004)); px = event.clientX; py = event.clientY; }); canvas.addEventListener('pointerup', event => { drag = false; canvas.releasePointerCapture(event.pointerId); if (dragDistance >= 5) return; const rect = canvas.getBoundingClientRect(), x = event.clientX - rect.left, y = event.clientY - rect.top; const clicked = [...nodes].reverse().find(node => { const point = lastProjected.get(node.id); return point && Math.hypot(point.x - x, point.y - y) <= (point.hitRadius || 7) + 7; }); if (!clicked) return; resetStepDemo(); if (derivationTitle && derivationCopy) { derivationTitle.textContent = '手动关系聚焦'; derivationCopy.textContent = '再次点击当前小球可取消聚焦，或点击“开始分步推导”恢复讲解。'; } focusNodeId = focusNodeId === clicked.id ? null : clicked.id; focusGroup = null; focusIndex = -1; focusedCache = null; overviewButton.classList.toggle('active', !focusNodeId); toolbarNote.textContent = focusNodeId ? `当前聚焦：${clicked.label}，相关画像依据和服务建议已高亮。` : '当前展示三个画像维度与三类组合接待方式的完整关系。'; }); canvas.addEventListener('wheel', event => { event.preventDefault(); zoom = Math.max(.6, Math.min(1.7, zoom * (event.deltaY > 0 ? .92 : 1.08))); }, {passive: false});
    resetButton.addEventListener('click', () => { rotX = -.12; rotY = -.3; zoom = 1; resetStepDemo(); focusGroup = null; focusNodeId = null; focusIndex = -1; focusedCache = null; overviewButton.classList.add('active'); toolbarNote.textContent = '三个画像维度分别连接三类接待方式，类别之间同时生效。'; });
    ...
    overviewButton.addEventListener('click', () => { resetStepDemo(); focusGroup = null; focusNodeId = null; focusIndex = -1; focusedCache = null; zoom = 1; overviewButton.classList.add('active'); toolbarNote.textContent = '当前展示三个画像维度与三类组合接待方式的完整关系。'; });
    nextButton.addEventListener('click', () => { const groups = [...dimensions.map(item => item.id), ...modeGroups.map(item => `mode_${item.id}`)]; if (!groups.length) return; resetStepDemo(); focusIndex = (focusIndex + 1) % groups.length; focusGroup = groups[focusIndex]; focusNodeId = null; focusedCache = null; zoom = 1.12; overviewButton.classList.remove('active'); const modeGroupId = focusGroup.replace(/^mode_/, ''); const label = zoneConfig[focusGroup]?.label || modeZoneConfig[modeGroupId]?.label || focusGroup; toolbarNote.textContent = `当前聚焦：${label}，相关画像依据和服务建议同步高亮。`; });
    replayButton?.addEventListener('click', () => { if (demoStep >= 4) { resetStepDemo(); toolbarNote.textContent = '可点击“开始分步推导”，逐段查看该号码画像如何形成接待建议。'; return; } demoStep += 1; demoActive = true; demoStart = performance.now(); demoPhase = -1; focusGroup = null; focusNodeId = null; focusIndex = -1; focusedCache = null; rotX = -.12; rotY = -.3; zoom = 1; overviewButton.classList.remove('active'); replayButton.classList.add('active'); replayButton.textContent = ['开始分步推导 →', '下一步：服务类别 →', '下一步：选择方式 →', '下一步：形成建议 →', '重新开始'][demoStep]; updateDerivationStatus(0); toolbarNote.textContent = `正在查看推导第 ${demoStep} 步；点击按钮继续下一环节。`; });
