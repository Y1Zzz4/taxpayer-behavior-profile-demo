(() => {
  ...
  function renderGraph(data) {
    ...
    const resultWrap = data ? el('div', 'result-trigger-wrap hidden') : null; const resultButton = data ? el('button', 'graph-tool graph-result-trigger', '生成接待建议') : null; const evidenceButton = data ? el('button', 'graph-tool', '复查推导依据') : null;
    [resetButton, rotationButton, labelsButton, overviewButton, nextButton, replayButton, fullReplayButton, resultButton, evidenceButton].filter(Boolean).forEach(button => button.type = 'button'); if (resultButton) { resultButton.setAttribute('aria-haspopup', 'dialog'); resultButton.setAttribute('aria-expanded', 'false'); resultWrap.append(resultButton, evidenceButton); }
    ...
    function openEvidenceDialog(lane, nextStep, reviewOnly=false) { if (!data) { applyDemoStep(nextStep); return; } clearEvidenceOverlay(); const content=evidenceContent(lane); const overlay=el('div','derivation-evidence-overlay'); overlay.setAttribute('role','dialog'); overlay.setAttribute('aria-modal','true'); const card=el('section','derivation-evidence-card'); card.style.setProperty('--evidence-color',content.color); card.append(el('small','',`${content.title} · 当前判断：${content.value}`),el('h2','',content.title),el('p','',content.basis)); content.sourceRows.forEach(row=>{const source=el('div','derivation-evidence-source');source.append(el('strong','',lane==='facts'?'历史事实发生来电':'判断所依据的历史来电'),el('span','',row.label));if(row.businessId){const detail=el('button','derivation-evidence-link','查看该通来电记录 ↗');detail.type='button';detail.addEventListener('click',()=>{if(fullReplayActive)stopFullReplay();window.clearTimeout(evidenceTimer);toolbarNote.textContent='已暂停自动推导，可在关闭来电详情后继续核对依据。';openDetail(row.businessId);});source.append(detail);}card.append(source);});const actions=el('div','derivation-evidence-actions');const close=el('button','graph-tool',reviewOnly?'关闭依据窗口':'依据已确认，继续推导 →');close.type='button';actions.append(close);card.append(actions);overlay.append(card);document.body.append(overlay);evidenceOverlay=overlay;requestAnimationFrame(()=>overlay.classList.add('open'));const continueFlow=()=>{if(evidenceOverlay!==overlay)return;clearEvidenceOverlay();if(reviewOnly)return;applyDemoStep(nextStep);if(fullReplayActive)scheduleAutoStep(nextStep);};close.addEventListener('click',continueFlow);close.focus({preventScroll:true});if(fullReplayActive&&!reviewOnly)evidenceTimer=window.setTimeout(continueFlow,1700); }
    function openEvidenceReview() { if (!data) return; clearOverlay(); const overlay=el('div','derivation-confirm-overlay'); overlay.setAttribute('role','dialog'); overlay.setAttribute('aria-modal','true'); const card=el('section','derivation-confirm-card'); card.append(el('span','derivation-confirm-eyebrow','推导依据复查'),el('h2','','选择要复查的判断依据'),el('p','','可随时回看各项判断来源，并直接打开对应的历史来电记录。')); const options=el('div','derivation-evidence-review-list'); [['proficiency','业务专业度依据'],['emotion','近期情绪依据'],['facts','历史服务事实']].forEach(([lane,label])=>{const button=el('button','graph-tool',label);button.type='button';button.addEventListener('click',()=>{clearOverlay();openEvidenceDialog(lane,0,true);});options.append(button);}); const close=el('button','graph-tool','关闭');close.type='button';close.addEventListener('click',clearOverlay); card.append(options,close);overlay.append(card);document.body.append(overlay);resultOverlay=overlay;requestAnimationFrame(()=>overlay.classList.add('open')); }
    ...
    function resetDemo(){stopFullReplay();demoActive=false;demoStep=0;demoPhase=-1;window.clearTimeout(modalTimer);clearOverlay();resultWrap?.classList.add('hidden');syncReplayButton();syncOverviewButton();updateStatus(0);}
    ...
    const keyHandler=event=>{if(event.key!=='Escape')return;const detailOverlay=document.querySelector('#detail-overlay');if(detailOverlay&&!detailOverlay.classList.contains('hidden')){event.preventDefault();closeDetail();return;}if(evidenceOverlay){event.preventDefault();stopFullReplay();clearEvidenceOverlay();toolbarNote.textContent='依据窗口已关闭；可继续分步推导或重新开始完整推导。';return;}if(!resultOverlay)return;event.preventDefault();if(resultOverlay.classList.contains('derivation-result-overlay')){closeResultCard();return;}clearOverlay();if(demoStep===8){resultWrap?.classList.remove('hidden');resultButton?.classList.add('available');resultButton?.focus({preventScroll:true});}};
    document.addEventListener('keydown',keyHandler);const observer=new ResizeObserver(resize);observer.observe(canvas);requestAnimationFrame(()=>{resize();draw();});state.graphCleanup=()=>{alive=false;cancelAnimationFrame(frame);stopFullReplay();window.clearTimeout(modalTimer);document.removeEventListener('keydown',keyHandler);clearOverlay();observer.disconnect();};
  }
